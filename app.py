...
BATCH_SIZE = 30
MARKET_LOAD_TIMEOUT = 120
SCAN_TIMEOUT = 30

# ================= FASTAPI APP =================
app = FastAPI(title="ArbiHunt", version="3.0")
    ...
    try:
        ...
        if exchange_id in cached_data:
            ...
            try:
                ...
                print(f"⏰ {exchange_id} TIMEOUT after {MARKET_LOAD_TIMEOUT}s")
                if exchange:
...
async def initialize_exchanges():
    ...
    print("\n" + "=" * 50)
    print("⚡ ARBIHUNT SCANNER - STARTING")
    print("=" * 50 + "\n")
    ...
    if exchanges_loaded >= 2 and len(all_symbols) > 0:
        ...
        print("⚠️ Not enough exchanges loaded. Will retry on next request.")
...
async def scan_symbol(symbol):
    try:
        ...
        for name, ex in exchanges.items():
            tasks.append(asyncio.wait_for(ex.fetch_order_book(symbol, limit=1), timeout=SCAN_TIMEOUT))
...
async def signup(request: Request):
    ...
    try:
        result = supabase.auth.sign_up({"email": data.get("email"), "password": data.get("password")})
...
async def login(request: Request):
    ...
    try:
        ...
        return {"success": True, "access_token": result.session.access_token, "user": profile.data}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def user_status(user_id: str):
    ...
    try:
        ...
        if profile.data:
            ...
            if is_pro and pro_expiry and datetime.now() > datetime.fromisoformat(pro_expiry):
                supabase.table("profiles").update({"is_pro": False}).eq("id", user_id).execute()
                is_pro = False
            return {"is_pro": is_pro, "pro_expiry": pro_expiry, "name": profile.data.get("name"), "email": profile.data.get("email")}
        return {"is_pro": False}
    except:
        return {"is_pro": False}
...
<head>
    ...
    <title>ArbiHunt - Arbitrage Scanner</title>
    <style>
        :root { --bg: #0D0D0D; --card: #141414; --border: #1F1F1F; --text: #E0E0E0; --text-secondary: #888; --green: #00C853; --red: #FF5252; --orange: #FF9800; --gold: #FFD700; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: var(--bg); color: var(--text); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; min-height: 100vh; -webkit-tap-highlight-color: transparent; }
        ...
        .btn { background: linear-gradient(135deg, #FFD700, #FF8F00); color: #000; font-weight: 700; font-size: 12px; padding: 8px 16px; border-radius: 20px; border: none; cursor: pointer; text-decoration: none; }
        .btn-outline { background: transparent; color: #fff; border: 1px solid var(--border); }
        
        .pro-banner { background: linear-gradient(135deg, rgba(255,152,0,0.15), rgba(255,152,0,0.05)); border: 1px solid rgba(255,152,0,0.3); border-radius: 12px; margin: 12px 16px; padding: 14px 16px; display: flex; align-items: center; gap: 12px; }
        .pro-banner .lock-text { flex: 1; font-size: 13px; color: var(--orange); font-weight: 600; }
        
        .stats { display: flex; gap: 8px; padding: 8px 16px; overflow-x: auto; }
        .stats::-webkit-scrollbar { display: none; }
        .stat { background: var(--card); border: 1px solid var(--border); border-radius: 20px; padding: 8px 14px; font-size: 11px; white-space: nowrap; color: var(--text-secondary); display: flex; align-items: center; gap: 6px; }
        ...
        .opp-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; margin-bottom: 10px; overflow: hidden; cursor: pointer; transition: all 0.2s; }
        ...
        .route { display: flex; align-items: center; gap: 6px; margin-bottom: 6px; flex-wrap: wrap; }
        .tag-buy { background: rgba(0,200,83,0.15); color: #00C853; padding: 3px 8px; border-radius: 6px; font-size: 10px; font-weight: 700; }
        .tag-sell { background: rgba(255,82,82,0.15); color: #FF5252; padding: 3px 8px; border-radius: 6px; font-size: 10px; font-weight: 700; }
        .symbol { font-size: 18px; font-weight: 700; color: #fff; }
        .meta { display: flex; gap: 12px; margin-top: 4px; font-size: 11px; color: var(--text-secondary); }
        .card-right { text-align: right; flex-shrink: 0; }
        .spread { font-size: 24px; font-weight: 800; color: #00C853; }
        .detail { display: none; border-top: 1px solid var(--border); background: #0F0F0F; padding: 16px; }
        .detail.show { display: block; }
        .detail-section { margin-bottom: 16px; }
        .detail-title { font-size: 14px; font-weight: 700; color: #fff; margin-bottom: 10px; }
        .detail-row { display: flex; justify-content: space-between; padding: 7px 0; font-size: 13px; border-bottom: 1px solid #1A1A1A; }
        .detail-label { color: #777; } .detail-val { color: #fff; font-weight: 500; }
        .warning { background: rgba(255,152,0,0.1); border-left: 3px solid var(--orange); padding: 10px 12px; border-radius: 6px; font-size: 11px; color: var(--orange); margin-top: 12px; }
        ...
        .plan-price { font-size: 20px; font-weight: 800; color: #FFD700; }
        .plan-desc { font-size: 11px; color: #888; }
    </style>
</head>
<body>
    <div class="header">
        ...
        <div style="display:flex;gap:8px;">
            <button class="btn" onclick="showProModal()" id="proBtn" style="display:none;">GO PRO</button>
            <button class="btn btn-outline" onclick="showAuthModal()" id="loginBtn">Sign In</button>
            <span id="userName" style="color:#FFD700;font-weight:600;display:none;cursor:pointer;" onclick="location.href='/profile'"></span>
        </div>
    </div>
    ...
    <div class="pro-banner" id="proBanner">
        ...
        <button class="btn" onclick="showProModal()">GO PRO</button>
    </div>

    <div class="stats">
        <div class="stat"><span class="live-dot"></span> LIVE</div>
        <div class="stat">📊 <span class="count" id="oppCount">0</span> opps</div>
        <div class="stat">🔄 Round <span class="count" id="roundNum">0</span></div>
    </div>

    <div class="opp-list" id="oppList">
        <div class="no-data"><div class="icon">⚡</div><div style="font-size:14px;">Scanning all USDT pairs...</div><div style="font-size:11px;color:#444;margin-top:6px;">Gate.io • KuCoin • MEXC • Bitget • CoinEx</div></div>
    </div>
    ...
    <div class="modal" id="authModal">
        <div class="modal-content">
            ...
            <div class="input-group" id="nameGroup" style="display:none;"><label>Name</label><input type="text" id="authName"></div>
            <div class="input-group" id="phoneGroup" style="display:none;"><label>Phone (2547XXXXXXXX)</label><input type="tel" id="authPhone"></div>
    ...
    <div class="modal" id="proModal">
        <div class="modal-content">
            ...
            <div class="input-group"><label>Safaricom Phone</label><input type="tel" id="mpesaPhone" placeholder="254712345678"></div>
...
<script>
    let opps = [], expandedCard = null, roundNum = 0, currentUser = null, selectedPlan = 'monthly';
    
    const stored = localStorage.getItem('arbihunt_user');
    ...
    function toggleCard(idx, e) {
        ...
        if (expandedCard === idx) { d.classList.remove('show'); expandedCard = null; }
        else {
            if (expandedCard !== null) document.getElementById(`detail-${expandedCard}`)?.classList.remove('show');
            d.classList.add('show'); expandedCard = idx;
        }
    }
    ...
    function render() {
        ...
        if (!opps.length) {
            c.innerHTML = '<div class="no-data"><div class="icon">⚡</div><div style="font-size:14px;">Scanning all USDT pairs...</div><div style="font-size:11px;color:#444;margin-top:6px;">Gate.io • KuCoin • MEXC • Bitget • CoinEx</div></div>';
            return;
        }
        c.innerHTML = opps.map((o,i) => `
            <div class="opp-card" onclick="toggleCard(${i},event)">
                <div class="card-header">
                    <div class="card-left">
                        <div class="route"><span class="tag-buy">BUY</span> ${fmtEx(o.buy_exchange)} → <span class="tag-sell">SELL</span> ${fmtEx(o.sell_exchange)}</div>
                        <div class="symbol">${o.symbol}/USDT</div>
                        <div class="meta"><span>💰 $${Number(o.liquidity).toLocaleString()}</span><span>⏱ ${ago(o.timestamp)}</span></div>
                    </div>
                    <div class="card-right"><div class="spread">${o.spread}%</div></div>
                </div>
                <div class="detail" id="detail-${i}">
                    <div class="detail-section">
                        <div class="detail-title">1️⃣ Buy at ${fmtEx(o.buy_exchange)}</div>
                        <div class="detail-row"><span class="detail-label">Price:</span><span class="detail-val">$${o.buy_price}</span></div>
                        <div class="detail-row"><span class="detail-label">Liquidity:</span><span class="detail-val">$${Number(o.buy_liquidity).toLocaleString()}</span></div>
                    </div>
                    <div class="detail-section">
                        <div class="detail-title">2️⃣ Sell at ${fmtEx(o.sell_exchange)}</div>
                        <div class="detail-row"><span class="detail-label">Price:</span><span class="detail-val">$${o.sell_price}</span></div>
                        <div class="detail-row"><span class="detail-label">Liquidity:</span><span class="detail-val">$${Number(o.sell_liquidity).toLocaleString()}</span></div>
                    </div>
                    <div class="detail-row"><span class="detail-label">Spread:</span><span class="detail-val" style="color:#00C853;">${o.spread}%</span></div>
                    <div class="detail-row"><span class="detail-label">Net Profit:</span><span class="detail-val">${o.net_profit}%</span></div>
                    <div class="warning">⚠️ Double check coin contract before trading. Opportunities last 10-15 minutes.</div>
                </div>
            </div>
        `).join('');
        expandedCard = null;
    ...
    async function checkPro() {
        ...
        try {
            const r = await fetch(`/api/user/status?user_id=${currentUser.id}`);
    ...
    function toggleAuthMode() {
        ...
        document.getElementById('authToggle').textContent = isSignUp ? 'Have account? Sign In' : 'Create Account';
    ...
    async function handleAuth() {
        ...
        const body = isSignUp ? { email, password, name: document.getElementById('authName').value, phone: document.getElementById('authPhone').value } : { email, password };
        
        try {
            const r = await fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) });
            const d = await r.json();
            if (d.success) {
                currentUser = d.user || { email, name: body.name, phone: body.phone, id: d.user_id };
                localStorage.setItem('arbihunt_user', JSON.stringify(currentUser));
                updateUI();
                closeModal('authModal');
            } else {
                document.getElementById('authError').textContent = d.error || 'Error';
            }
        } catch(e) { document.getElementById('authError').textContent = 'Network error'; }
    }
    
    function showProModal() { document.getElementById('proModal').classList.add('show'); if (currentUser?.phone) document.getElementById('mpesaPhone').value = currentUser.phone; }
    ...
    async function payWithMpesa() {
        ...
        if (!phone) { document.getElementById('payStatus').innerHTML = '<span style="color:#FF5252;">Enter phone number</span>'; return; }
        ...
        try {
            const r = await fetch('/api/mpesa/stkpush', {
                ...
                body:JSON.stringify({ phone, plan: selectedPlan, user_id: currentUser?.id || '' })
            });
            const d = await r.json();
            if (d.success) {
                document.getElementById('payStatus').innerHTML = '<span style="color:#00C853;">✅ Prompt sent! Check your phone and enter PIN.</span>';
            } else {
                document.getElementById('payStatus').innerHTML = `<span style="color:#FF5252;">Error: ${d.error}</span>`;
            }
        } catch(e) { document.getElementById('payStatus').innerHTML = '<span style="color:#FF5252;">Network error</span>'; }
    }
    
    // Poll for opportunities every 3 seconds
    async function pollOpps() {
        try {
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { position: sticky; top: 0; z-index: 100; background: var(--bg); padding: 12px 16px; border-bottom: 1px solid var(--border); display: flex; align-items: center; }
        .logo { font-size: 22px; font-weight: 800; background: linear-gradient(135deg, #FFD700, #FFA000); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .chat-container { padding: 12px 16px 160px; }
        .msg { margin-bottom: 10px; animation: fadeIn 0.3s; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .msg-name { font-size: 11px; font-weight: 700; color: #FFD700; margin-bottom: 2px; }
        .msg-text { background: var(--card); padding: 10px 14px; border-radius: 12px; font-size: 14px; display: inline-block; max-width: 80%; }
        ...
        .input-bar button { background: linear-gradient(135deg, #FFD700, #FF8F00); color: #000; border: none; border-radius: 20px; padding: 10px 18px; font-weight: 700; cursor: pointer; }
        .bottom-nav { position: fixed; bottom: 0; left: 0; right: 0; background: var(--bg); border-top: 1px solid var(--border); display: flex; justify-content: space-around; padding: 10px 0; padding-bottom: max(10px, env(safe-area-inset-bottom)); z-index: 100; }
        .nav-item { display: flex; flex-direction: column; align-items: center; gap: 4px; color: #555; font-size: 10px; font-weight: 600; text-decoration: none; }
        .nav-item.active { color: #FFD700; }
        .nav-icon { font-size: 20px; }
    </style>
</head>
<body>
    ...
    <div class="chat-container" id="chatContainer"><div style="text-align:center;color:#444;padding:40px;">Loading messages...</div></div>
    <div class="input-bar">
        <input type="text" id="chatInput" placeholder="Share your story..." onkeypress="if(event.key=='Enter')sendMsg()">
...
<script>
    ...
    function addMsg(name, text, time) {
        ...
        d.className = 'msg';
        d.innerHTML = `<div class="msg-name">${name}</div><div class="msg-text">${text}</div><div class="msg-time">${t}</div>`;
        c.appendChild(d);
    ...
    async function sendMsg() {
        ...
        if (!msg) return;
        input.value = '';
        addMsg(userName, msg, new Date().toISOString());
        await fetch('/api/chat/send', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ user_id: userId, user_name: userName, message: msg }) });
    }
    ...
    async function loadMsgs() {
        try {
            ...
            const d = await r.json();
            const c = document.getElementById('chatContainer');
            if (d.messages && d.messages.length) { c.innerHTML = ''; d.messages.forEach(m => addMsg(m.user_name, m.message, m.created_at)); }
        } catch(e) {}
    }
...
<body>
    ...
    <div class="profile-card">
        ...
        <div class="name" id="pname">Loading...</div>
        <div class="email" id="pemail"></div>
        <div id="pstatus"></div>
    </div>
    <div class="info-row"><span class="info-label">Account ID</span><span id="pid">-</span></div>
...
<script>
    const user = JSON.parse(localStorage.getItem('arbihunt_user') || '{}');
    if (!user.email) location.href = '/';
    document.getElementById('pname').textContent = user.name || 'User';
    document.getElementById('pemail').textContent = user.email || '';
    document.getElementById('av').textContent = (user.name || 'U')[0].toUpperCase();
    document.getElementById('pid').textContent = user.id || 'N/A';
    
    async function checkPro() {
        if (!user.id) return;
        try {
            const r = await fetch(`/api/user/status?user_id=${user.id}`);
            const d = await r.json();
            if (d.is_pro) document.getElementById('pstatus').innerHTML = '<span class="pro-badge">⭐ PRO Member</span>';
        } catch(e) {}
    }
    checkPro();
    
    function logout() { localStorage.removeItem('arbihunt_user'); location.href = '/'; }
</script>
</body>
</html>
"""

# Vercel uses the app directly - no uvicorn.run() needed
